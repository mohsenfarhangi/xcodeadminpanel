...
class Theme {
    ...
    */
    public static function init($theme, $demo = '') {
        ...
        // For multi demo, merge bemoonim menu config with current demo
        if (self::$demo != '' && self::$demo != 'bemoonim') {
            $demo1_actual_path = str_replace(self::$demo, 'bemoonim/', $_THEME_PATH);
    ...
    */
    ...
    public static function getView($path, $params = array()) {
        ...
        // For multi demo, include from bemoonim for other demos
        if ( file_exists($actual_path) === false ) {
            if (self::$demo != '' && self::$demo != 'bemoonim') {
                $actual_path = str_replace(self::$demo . '/', 'bemoonim/', $actual_path);
            }
        }
    ...
    public static function getChangelogInfo() {
        ...
        if ( file_exists($current_path) === false ) {
            if (self::$demo != '' && self::$demo != 'bemoonim') {
                $current_path = str_replace(self::$demo . '/', 'bemoonim/', $current_path);
            }
        }
